(ns anvil.dispatcher.native-rpc-handlers.users.v2.twilio
  (:require [slingshot.slingshot :refer :all]
            [org.httpkit.client :as http]
            [anvil.runtime.conf :as runtime-conf]
            [anvil.util :as util]
            [clojure.tools.logging :as log]
            [anvil.dispatcher.native-rpc-handlers.users.twilio :as users-v1-twilio]
...
(defn get-mfa-method-for-current-user []
  (let [{:keys [table_id]} (users-util/get-user-props)
    ...
    (->> (users-util/get-in-user-row user-row "mfa")
...
(defn generate-mfa-method [_kwargs phone]
  {:type  "twilio-verify"
   :id    (random/base32 5)
   :serial 1
   :phone (secrets/encrypt-str-with-global-key ::users-v1-twilio/phone phone)})
    ...
    (let [{:keys [phone]} (or mfa-method
          ...
          {:keys [body status] :as verification} @(http/post (str "https://verify.twilio.com/v2/Services/" verify-service-id "/Verifications")
                                                             {:headers     {"Authorization" (util/basic-auth-header account-sid auth-token)}
                                                              :form-params {"To"      (secrets/decrypt-str-with-global-key ::users-v1-twilio/phone phone)
                                                                            "Channel" channel}})]
...
(defn v1-users-handlers []
  (require 'anvil.dispatcher.native-rpc-handlers.users.twilio)
  ((resolve 'anvil.dispatcher.native-rpc-handlers.users.twilio/v1-users-handlers)))
