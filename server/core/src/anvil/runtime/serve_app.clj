...
(defn serve-app [{:keys [app-id environment environment-from-url app-session] :as req} client-params {:keys [action print-id print-key form-name form-arg-json replace-url] :as what-to-do}]
...
(defn- with-safe-anvil-cookies [resp req headers anvil-cookies-updated?]
  (let [{:keys [app-session cross-origin]} req
        resp-with-safe-headers (reduce (fn [resp [k v]]
                           ...
                           (binding [rpc-util/*environment* environment]
                             (try+
                               (cond
                                 ...
                                 (and (instance? SerializedPythonObject body) (= (:type body) "anvil.server.FormResponse"))
                                 (if-not (= request-category :route)
                                   (throw+ {:anvil/server-error "Cannot return a FormResponse from an anvil.server.http_endpoint."})
                                   (let [{:keys [form_name args kwargs] :as v} (:value body)]
                                     (serialiser/serialise-to-map-with-media
                                       {:args args :kwargs kwargs :id "DUMMY"}
                                       (fn [arg-json arg-media]
                                         (send! channel
                                                (wrap-response
                                                  (serve-app request client-params
                                                             {:action        :open-form
                                                              :form-name     form_name
                                                              :form-arg-json (-> arg-json
                                                                                 (dissoc :id)
                                                                                 (assoc :media arg-media))}))))
