(ns anvil.runtime.serve-app.core
  ...
  (:import (anvil.dispatcher.types Media MediaDescriptor InputStreamMedia ChunkedStream SerializedPythonObject)
  ...
  (util/timeit "Serve app" [checkpoint!]
    (let [environment (or environment-from-url environment) ;; Ignore overrides from the session on reload
      ...
      (if-let [error-message (app-data/should-app-be-blocked? app-id app-session environment)]
        (block-util/block-app req error-message)
        (if (accounting/limit? app-id)
          (block-util/block-app req (str "This app cannot be accessed due to plan limits being exceeded."))
          (do
...
(defn serve-request-to-app-url [{:keys [app-id app-session environment uri remote-addr] :as request} client-params wrap-response]
  ...
  (try+
    (if-let [error-message (app-data/should-app-be-blocked? app-id app-session environment)]
      (block-util/block-app request error-message)
      (let [app-info (app-data/get-app-info-insecure app-id)
            ...
            ip-whitelist (get-in app-info [:hard-coded :ip-whitelist])]
        (if (and ip-whitelist (not (some #(block-util/ip-matches-whitelist-entry? remote-addr %) ip-whitelist)))
          (do
            (log/warn (str "Access denied to admin app from IP: " remote-addr " (not in whitelist)"))
            (resp/status (resp/response "Access denied") 403))
          (cond
