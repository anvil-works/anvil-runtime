...
(defn get-login-response [req state client-id redirect-uri scope request-offline]
  (let [csrf-token   (random/hex 60)
    ...
    (->
     ...
     (ring.util.response/redirect (str "https://accounts.google.com/o/oauth2/auth?"
     ...
     ;; Store the CSRF token in the local session.
     (assoc :session (assoc (:session req) ::csrf-token csrf-token)))))
...
(defn refresh-access-token [refresh-token client-id client-secret]
  (let [params {:keepalive -1
        ...
        body (assoc body :expires_at (Date. (+ (.getTime (Date.)) (* 1000 (or (:expires_in body) 0)))))]
