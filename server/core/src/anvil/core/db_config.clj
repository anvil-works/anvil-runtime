...
(defn set-val [key value]
  (let [key-name (util/preserve-slashes key)
        ...
        val (if (bytes? value)
              ...
              {:val value})]
    (jdbc/execute! util/db ["INSERT INTO anvil_config (key, value) VALUES (?, ?::jsonb)
                             ON CONFLICT (key) DO UPDATE SET value=?::jsonb WHERE anvil_config.key=?"
                            key-name val val key-name]))
  value)

(defn get-val [key]
  (when-let [{:keys [type val]} (:value (first (jdbc/query util/db ["SELECT value FROM anvil_config WHERE key = ?" (util/preserve-slashes key)])))]
    (if (= type "binary")
