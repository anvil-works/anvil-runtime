...
import {
    ...
    hashString,
    serializeFormLayout,
    serializeFormLayoutWithResult,
    createDeterministicDropzoneNameGenerator,
    type ParsedLayoutHtml,
...
function hasComponentNamed(components: ParsedLayoutHtml["components_by_slot"][string] | undefined, name: string): boolean {
    return (components ?? []).some((component) => component.name === name || hasComponentNamed(component.components, name));
}

function makeLayoutReparseFixture(): ParsedLayoutHtml {
    return {
        layout: {
...
describe("parseLayoutHtml", () => {
    ...
    it("round-trips a layout form that re-exposes a slot from its layout", () => {
        const parsed: ParsedLayoutHtml = {
            layout: { type: "form:BaseLayout" },
            components_by_slot: {
                slot_1: [],
            },
            slots: {
        ...
        const serialized = serializeFormLayout(parsed);

        expect(normalizeMultiline(serialized)).toBe(
            normalizeMultiline(`<anvil-form layout="form:BaseLayout">
    <anvil-block slot="slot_1">
    ...
    it("serializes a slot-only layout form when components_by_slot is omitted", () => {
        const form: FormYaml = {
            class_name: "Layout",
            code: "",
            is_package: true,
        ...
        const serialized = serializeFormLayout(form);
    ...
    <anvil-block slot="slot_1">
        ...
        expect(parseLayoutForm(serialized)).toEqual({
            layout: { type: "form:BaseLayout" },
            components_by_slot: { slot_1: [] },
            slots: form.slots,
        });
    });

    it("preserves a re-exposed layout slot when structural reparse promotes all DOM nodes", () => {
        const parsed: ParsedLayoutHtml = {
            layout: { type: "form:BaseLayout" },
            components_by_slot: {
                slot_1: [
                    {
                        ...
                        name: "fragment_1",
                        properties: {
                            html: `<section><p>Intro</p></section>`,
                        },
                        components: [
                            {
                                type: "Button",
        ...
        const result = serializeFormLayoutWithResult(parsed, {
            allowReparse: true,
            parserOptions: { domNodePromotion: "all" },
        });

        expect(result.structuralHtmlChanged).toBe(true);
        expect(normalizeMultiline(result.html)).toContain(`<anvil-block slot="slot_1">`);
        expect(normalizeMultiline(result.html)).toContain(`<anvil-slot name="slot_1"></anvil-slot>`);
        expect(parsed.layout).toEqual({ type: "form:BaseLayout" });
        expect(parsed.slots?.slot_1?.target).toEqual({ type: "slot", name: "slot_1" });
        expect(hasComponentNamed(parsed.components_by_slot.slot_1, "button_1")).toBe(true);

        const reparsed = parseLayoutForm(result.html, { domNodePromotion: "all" });

        expect(reparsed.layout).toEqual(parsed.layout);
        expect(reparsed.slots?.slot_1?.target).toEqual({ type: "slot", name: "slot_1" });
        expect(hasComponentNamed(reparsed.components_by_slot.slot_1, "button_1")).toBe(true);
    });
...
>
    ...
    it("reports structural change when nested fragment serialization reparses in layouts", () => {
        const parsed = makeLayoutReparseFixture();

        const result = serializeFormLayoutWithResult(parsed, { allowReparse: true });
    ...
    it("preserves a layout block slot when its dropzone is missing during structural serialization", () => {
        const parsed: ParsedLayoutHtml = {
            layout: { type: "form:layouts.dialog" },
            components_by_slot: {
                body: [],
            },
            slots: {
        ...
        expect(normalizeMultiline(result.html)).toContain(
            normalizeMultiline(`<anvil-block slot="body">
        <anvil-slot name="content" container:dropzone="$dz_missing"></anvil-slot>
    </anvil-block>`)
        );
        expect(parsed.slots?.content).toBeDefined();
        expect(parsed.slots?.content?.set_layout_properties?.dropzone).toBe("$dz_missing");
    });

    it("does not reparse layout structural changes unless allowReparse is enabled", () => {
        const parsed = makeLayoutReparseFixture();

        const result = serializeFormLayoutWithResult(parsed, { allowReparse: false });

        expect(result.structuralHtmlChanged).toBe(false);
        expect(parsed.components_by_slot.body[0].components?.[0].layout_properties?.dropzone).toBe("$dz_missing");
        expect(parsed.components_by_slot.body).toHaveLength(1);
    });

    it("keeps serializeFormLayout as a string-returning wrapper", () => {
        const parsed = makeLayoutReparseFixture();

        const serialized = serializeFormLayout(parsed);

        expect(typeof serialized).toBe("string");
        expect(serialized).toBe(serializeFormLayoutWithResult(parsed, { allowReparse: false }).html);
        expect(parsed.components_by_slot.body[0].components?.[0].layout_properties?.dropzone).toBe("$dz_missing");
    });

    it("reparses through serializeFormLayout when explicitly requested", () => {
        const parsed = makeLayoutReparseFixture();

        const serialized = serializeFormLayout(parsed, { allowReparse: true });

        expect(typeof serialized).toBe("string");
        expect(parsed.components_by_slot.body[0].components?.[0].layout_properties?.dropzone).not.toBe("$dz_missing");
    });
