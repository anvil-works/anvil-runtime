...
<anvil-component type="Button" name="button_1" prop:text="button_1"></anvil-component>`;
    ...
    it("syncs generated HtmlComponent dropzones after deleting parsed top-level components", () => {
...
<anvil-component type="Button" name="button_2" prop:text="button_2"></anvil-component>`;
        ...
        parsed.components = parsed.components.filter((component) => component.name !== "button_1");
        const serializedAfterFirstDelete = serializeFormContainer(parsed, { allowReparse: true });

        expect(normalizeMultiline(serializedAfterFirstDelete)).toBe(
            normalizeMultiline(`<p>Before</p>
<anvil-component type="Button" name="button_2" prop:text="button_2"></anvil-component>`)
        );
        const remainingDropzone = parsed.components[0]?.layout_properties?.dropzone;
        expect(remainingDropzone).toBeDefined();
        expect(parsed.container.properties?.html?.match(/<anvil-dropzone /g)).toHaveLength(1);
        expect(parsed.container.properties?.html).toContain(remainingDropzone);

        parsed.components = [];
        const serializedAfterFinalDelete = serializeFormContainer(parsed, { allowReparse: true });

        expect(serializedAfterFinalDelete).toContain("<p>Before</p>");
        expect(serializedAfterFinalDelete.match(/<anvil-dropzone /g)).toHaveLength(1);
        expect(parsed.container.properties?.html?.match(/<anvil-dropzone /g)).toHaveLength(1);
        expect(JSON.stringify(parsed)).not.toContain("generated-component-dropzones");
    });

    it("preserves explicit empty dropzones when generated component dropzones are synced", () => {
        const html = `<anvil-dropzone name="explicit"></anvil-dropzone>
<anvil-component type="Button" name="button_1" prop:text="button_1"></anvil-component>
<anvil-component type="Button" name="button_2" prop:text="button_2"></anvil-component>`;
        ...
        expect(serialized).toContain('<anvil-dropzone name="explicit"></anvil-dropzone>');
        expect(serialized).toContain(
            '<anvil-component type="Button" name="button_2" prop:text="button_2"></anvil-component>'
        );
        expect(serialized.match(/<anvil-dropzone /g)).toHaveLength(1);
        expect(parsed.container.properties?.html).toContain('<anvil-dropzone name="explicit"></anvil-dropzone>');
        expect(parsed.container.properties?.html?.match(/<anvil-dropzone /g)).toHaveLength(2);
    });

    it("keeps an explicit nested dropzone when deleting the adjacent generated component dropzone", () => {
        const html = `<center style="font-style: italic; color: #888; margin: 3em">
  (Edit this HTML in the HTML pane is a thing)
  <anvil-dropzone name="foo"></anvil-dropzone>
  <anvil-component type="Button" name="button_2" prop:text="button_2"></anvil-component>
</center>`;
        ...
        expect(serialized).toContain('<anvil-dropzone name="foo"></anvil-dropzone>');
        expect(serialized).not.toContain("$dz_");
        expect(serialized.match(/<anvil-dropzone /g)).toHaveLength(1);
        expect(parsed.container.properties?.html).toContain('<anvil-dropzone name="foo"></anvil-dropzone>');
        ...
        expectNormalizedComponents(reparsed.components, [
            {
                ...
                name: "$component_1",
                properties: {
                    html: '<div style="border: 1px solid blue; min-height: 20px"></div>',
                },
            },
            {
                type: "HtmlComponent",
