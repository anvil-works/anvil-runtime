...
${sections}
    ...
    it("preserves void element attributes through structural reparse", () => {
        const html = `<div class="page-shell">
  <main class="page-main">
    <img src="/asset.png" alt="Logo" width="100" loading="lazy">
    <anvil-slot name="content"></anvil-slot>
  </main>
</div>`;

        const parserOptions = { extractRootStyling: true, domNodePromotion: "all" as const };
        const parsed = parseSerializedHtml(html, parserOptions);
        const result = serializeFormContainerWithResult(parsed, {
            allowReparse: true,
            indentSize: 2,
            parserOptions,
        });
        ...
        expect(result.html).toContain('<img src="/asset.png" alt="Logo" width="100" loading="lazy" />');
    });
});
