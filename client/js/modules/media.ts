import type { Args, Kws, pyObject, pyType } from "@Sk";
import {
    buildNativeClass,
    chainOrSuspend,
    checkNone,
...
interface TempUrlSelf extends pyObject {
...
const media = () => {
    const mediaMod: Record<string, pyObject> = {
        __name__: new pyStr("media"),
        TempFile: new pyFunc(() => {
            throw new pyRuntimeError("Cannot create files from Media objects on the client.");
        }),
        from_file: new pyFunc(() => {
            throw new pyRuntimeError("Cannot create Media objects from files on the client.");
        }),
    };

    setUpModuleMethods("media", mediaMod, {
        ...
        download: {
            ...
            $meth: (media) =>
                chainOrSuspend(PyDefUtils.getUrlForMedia(media), (urlHandle) => {
                    ...
                    const navigator = window.navigator as NavigatorWithLegacySave;
                    if (navigator.msSaveOrOpenBlob && urlHandle.blob) {
                        navigator.msSaveOrOpenBlob(urlHandle.blob, filename);
                        urlHandle.release();
                    } else {
                        const a = document.createElement("a");
                        document.body.appendChild(a);
    ...
    mediaMod["TempUrl"] = buildNativeClass("anvil.media.TempUrl", {
        constructor: function TempUrl() {},
        slots: {
            tp$init(this: TempUrlSelf, args: Args, kws?: Kws) {
                checkOneArg("TempUrl", args);
                const [media] = args;
                if (!isTrue(pyIsInstance(media, AnvilMedia as pyType))) {
                    throw new pyTypeError(`expected a Media object, got ${typeName(media)}`);
                }
                this.handle = PyDefUtils.getUrlForMedia(media, kws) as MediaUrlHandle;
            },
        },
        methods: {
            ...
            __exit__: {
                $name: "__exit__",
                $meth(this: TempUrlSelf, excType: pyObject) {
                    this.handle.release();
