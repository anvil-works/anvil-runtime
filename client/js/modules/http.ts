import type { pyCallable, pyObject } from "@Sk";
import {
    buildPyClass,
    checkString,
    promiseToSuspension,
...
import { PyModMap, anvilMod, anvilServerMod } from "@runtime/runner/py-util";
...
type HttpBody = string | Uint8Array | URLSearchParams | Record<string, string> | undefined;

interface HttpRequestKwargs {
    url?: string;
    method?: string;
    username?: string;
    password?: string;
    headers?: HttpHeaders;
    json?: boolean;
    data?: HttpBody | unknown;
    timeout?: number;
}

const http = () => {
    const pyMod: PyModMap = { __name__: new pyStr("http") };

    pyMod["url_encode"] = pyMod["encode_uri_component"] = new pyFunc(function (c: pyStr) {
    ...
    pyMod["UrlEncodingError"] = buildPyClass(pyMod, ($gbl, $loc) => {}, "UrlEncodingError", [pyException]);
    pyCall(anvilServerMod["_register_exception_type"], [
        new pyStr("anvil.http.UrlEncodingError"),
        pyMod["UrlEncodingError"],
    ]);

    pyMod["url_decode"] = new pyFunc(function (s: pyStr) {
    ...
    pyMod["json_stringify"] = new pyFunc((obj: pyObject) => toPy(JSON.stringify(toJs(obj))));

    pyMod["b64_encode"] = new pyFunc((s: pyStr) => toPy(btoa(toJs(s))));

    pyMod["b64_decode"] = new pyFunc((s: pyStr) => toPy(atob(toJs(s))));

    const ExceptionInit = pyException.tp$getattr<pyCallable>(pyStr.$init)!;

    pyMod["HttpError"] = buildPyClass(
        pyMod,
        ($gbl, $loc) => {
            $loc["__init__"] = new pyFunc(function init(
                self: pyObject,
                pyStatus: pyObject | undefined,
                pyContent: pyObject | undefined,
                pyMessage: pyObject | undefined
            ) {
                pyStatus ??= pyNone;
                pyContent ??= pyNone;
                pyMessage ??= pyNone;
    ...
    function pyGetResponse(xhr: XMLHttpRequest, json: boolean | undefined, fail = true) {
        const r = xhr.response;
        if (!(r instanceof ArrayBuffer)) {
            return pyNone;
        }
        let bytes: pyObject = new pyBytes(new Uint8Array(r));
        if (json) {
            const decode = bytes.tp$getattr<pyCallable>(new pyStr("decode"))!;
            const jsstr = pyCall(decode, []).toString();
            let json: unknown = jsstr;
            try {
    ...
    function request(kws: HttpRequestKwargs, pyUrl?: pyStr) {
        if (pyUrl) {
        ...
        if (!hasContent) {
            try {
                ...
                _URL.search = _URL.search || (body as string);
                url = _URL.toString();
            } catch (e) {
                console.error(e);
            }
        ...
        const onError = (statusText?: string) => {
            ...
            let message: string | null = statusText || xhr.statusText;
