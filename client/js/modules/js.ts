import type { Args, Kws, pyCallable, pyExternalErrorConstructor, pyObject, pyProxy, pyType } from "@Sk";
import {
    chainOrSuspend,
    isTrue,
    lookupSpecial,
...
/** because we return undefined specifically when a python callback errors - see docs for report_exceptions */
const jsUndefinedReturn = undefined as unknown as pyObject;

const js = () => {
    const pyMod: Record<string, pyObject> = { __name__: new pyStr("js") };

    const call_js = PyDefUtils.callJs.bind(null, null as unknown as undefined);
    const dummyProxy = proxy({});
    const newProxy = lookupSpecial<pyCallable>(dummyProxy, pyStr.$new)!;
    const ProxyType = dummyProxy.constructor as unknown as pyType<pyProxy>;

    async function getModule(url: pyObject) {
        ...
        let jsUrl = url.toString();
        if (jsUrl.startsWith("./_/theme/")) {
            jsUrl = jsUrl.slice(1);
        }
        if (jsUrl.startsWith("/_/theme/")) {
            jsUrl = window.anvilAppOrigin + jsUrl;
        }
        const mod = await new Function("return import(" + JSON.stringify(jsUrl) + ")").call(undefined);
        return proxy(mod);
    }
    ...
    ProxyType.prototype.__serialize__ = new pyMethodDescriptor(ProxyType, {
        $name: "__serialize__",
        $meth(_globals: pyObject) {
            const as_js = this.valueOf() as Record<string, unknown> & { constructor: unknown };
    ...
    if (!ANVIL_IN_DESIGNER) {
        ...
        pyCall(anvilServerMod.portable_class, [ProxyType as unknown as pyObject, new pyStr("anvil.js.ProxyType")]);
    }

    function exceptionReporter(e: unknown) {
        window.onerror(null, null, null, null, e);
    }

    function exceptionReporterReRaise(e: unknown) {
        exceptionReporter(e);
        throw e;
    }
    ...
    setUpModuleMethods("js", pyMod, {
            ...
            $meth(wrappedPromise: pyObject) {
                ...
                if (
                    ...
                    return chainOrSuspend(promiseToSuspension(maybePromise), (res) =>
                        toPy(res, { dictHook: (obj: object) => proxy(obj) })
                    );
                }
        ...
        call_$rw$: {
            ...
            $meth(args) {
                ...
                return call_js(...(args as [pyStr, ...pyObject[]]));
            },
        ...
        to_media: {
            $name: "to_media",
            $meth(pyBlob: pyObject, pyType?: pyStr, pyName?: pyStr) {
                let blob = toJs(pyBlob);
                let type = toJs(pyType);
                const name = toJs(pyName);
                if (blob instanceof Blob) {
                    if (blob instanceof File) {
                        // special case
                        return pyCallOrSuspend(anvilMod.FileMedia, [blob as unknown as pyObject]);
                    }
                    // else pass
                } else if (Array.isArray(blob)) {
                    // assume they've passed an array of sensible types
                    type || (type = (blob[0] as { type?: string } | undefined)?.type);
                    blob = new Blob(blob as BlobPart[], { type });
                } else if (
                    [ArrayBuffer, Uint8Array].includes(
                        (blob as { constructor: ArrayBufferConstructor | Uint8ArrayConstructor }).constructor
                    )
                ) {
                ...
                const args = name
                    ? ([blob as unknown as pyObject, name as unknown as pyObject] as Args)
                    : ([blob as unknown as pyObject] as Args);
                // use internal call to BlobMedia where the first argument can be a BlobMedia object
                // the name can also be a js string
                return pyCallOrSuspend(anvilMod.BlobMedia, args);
            },
            ...
            $flags: {
                ...
                Defaults: [pyStr.$empty, pyNone], // gendoc doesn't like pyStr.$empty, pyNone because it can't scope the vars
            },
        },
        ...
        report_exceptions: {
            ...
            $meth(pyfunc: pyObject) {
                function callback_handler(args: Args, kws?: Kws) {
                    return tryCatchOrSuspend(
                        () => pyCallOrSuspend(pyfunc, args, kws),
                        (e) => {
                            exceptionReporter(e);
                            return jsUndefinedReturn;
                        }
                    );
                }
        ...
        report_all_exceptions: {
            ...
            $meth(reporting: pyObject, reraise: pyObject) {
                if (!isTrue(reporting)) {
    ...
    pyMod.ExternalError = pyExternalError;
    /*!defMethod(_,)!2*/ ({
    ...
    (pyMod.ExternalError as pyExternalErrorConstructor).prototype.original_error = new pyProperty(
        new pyFunc((self: pyExternalError) => {
            return toPy(self.nativeError, { dictHook: (obj: object) => proxy(obj) });
        })
    );
