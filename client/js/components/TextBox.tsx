...
import { checkNone, isTrue, pyFunc, pyNone, pyObject, pyStr, toJs, toPy } from "@Sk";
...
interface TextBoxAnvil {
    elements: { root: HTMLInputElement };
    inputType: string;
    hiddenInput: boolean;
    lastChangeVal: string;
    updateType: () => void;
}

interface TextBox extends ClassicComponent<TextBoxAnvil> {}

const TextBoxFactory = (pyModule: PyModMap) => {
    const ClassicComponent = pyModule["ClassicComponent"] as ClassicComponentConstructor;

    pyModule["TextBox"] = PyDefUtils.mkComponentCls<TextBox>(pyModule, "TextBox", {
        properties: PyDefUtils.assembleGroupProperties<TextBox>(
                ...
                text: {
                    ...
                    get(self, e) {
                        if (self._anvil.inputType === "number") {
                            const r = Number.parseFloat(e.value);
                            return toPy(isNaN(r) ? null : r);
                        } else {
                            return new pyStr(e.value);
                        }
                    },
                    set(s, e, pyV) {
        ...
        locals($loc) {
            $loc["__new__"] = PyDefUtils.mkNew<TextBox>(ClassicComponent, (self) => {
                self._anvil.element
                    .on("propertychange change keyup paste input", function (e) {
                        if (e.type !== "change")
                            // Happens just before we tab away
                            this.focus();
                        const lc = self._anvil.elements.root.value;
                ...
                self._anvil.updateType = () => {
                    self._anvil.elements.root.setAttribute(
                        "type",
                        self._anvil.hiddenInput ? "password" : self._anvil.inputType
                    );
                };
            });

            /*!defMethod(_)!2*/ ("Set the keyboard focus to this TextBox");
            $loc["focus"] = new pyFunc(function focus(self: TextBox) {
            ...
            $loc["select"] = new pyFunc(function select(
                ...
                if (pySelectionStart && pySelectionEnd) {
                    let selectionStart = toJs(pySelectionStart) as number | null;
                    let selectionEnd = toJs(pySelectionEnd) as number | null;
                    let direction = pyDirection
                        ? (toJs(pyDirection) as "forward" | "backward" | "none" | undefined)
                        : undefined;
                    self._anvil.elements.root.setSelectionRange(selectionStart, selectionEnd, direction);
                } else {
