...
interface SectionProps {
    panelId: string;
    full_width_row: boolean;
    row_background: pyObject | undefined;
}

const Section = ({ panelId, full_width_row, row_background }: SectionProps) => {
    ...
    return (
        <div refName="section" className={`anvil-panel-section belongs-to-${panelId}`} style={background}>
            <div
                refName="sectionContainer"
                className={`anvil-panel-section-container anvil-container-overflow belongs-to-${panelId} ${fwrClass}`}>
...
const Row = ({ rowId, panelId, wrap_on }: RowProps) => {
    const prefix = getCssPrefix();
    wrap_on = prefix + "wrap-" + wrap_on.toString();
    return (
        <div
            refName="row"
            data-anvil-row-id={rowId}
            className={`anvil-panel-row anvil-panel-row-${rowId} belongs-to-${panelId} ${wrap_on}`}></div>
    );
};
...
const Column = ({ colId, panelId, wrap_on }: ColumnProps) => {
    const prefix = getCssPrefix();
    wrap_on = prefix + "wrap-" + wrap_on.toString();
    return (
        <div
            refName="col"
            data-anvil-col-id={colId}
            className={`anvil-panel-col anvil-panel-col-${colId} belongs-to-${panelId} ${wrap_on}`}></div>
    );
};

class PanelContainer {
    columnPanel: ColumnPanel;
    parent: PanelContainer | null;
    children: number;
    el: HTMLElement;
    slotEl: HTMLElement;

    constructor(columnPanel: ColumnPanel, el: HTMLElement, slotEl: HTMLElement | null = null) {
    ...
    appendChildContainer(childContainer: PanelContainer) {
        this.children++;
...
class ColContainer extends PanelContainer {
    ...
    constructor(columnPanel: ColumnPanel, colId: string, props: any) {
        const [el] = (<Column colId={colId} {...props} />) as unknown as [HTMLElement];
...
class RowContainer extends PanelContainer {
    ...
    constructor(columnPanel: ColumnPanel, rowId: string, props: any) {
        const [el] = (<Row rowId={rowId} {...props} />) as unknown as [HTMLElement];
        super(columnPanel, el);
...
class SectionContainer extends PanelContainer {
    constructor(columnPanel: ColumnPanel, props: any) {
        const [section, sectionElements] = (<Section {...props} />) as unknown as [
            HTMLElement,
            { sectionGutter: HTMLElement }
        ];
        super(columnPanel, section, sectionElements.sectionGutter);
    }
}

export interface ColumnPanelAnvil {
    panelId: string;
...
export interface ColumnPanel extends ClassicContainer<ColumnPanelAnvil> {}

const ColumnPanelFactory = (pyModule: PyModMap) => {
    const ClassicContainer = pyModule["ClassicContainer"] as ClassicComponentConstructor;

    pyModule["ColumnPanel"] = PyDefUtils.mkComponentCls<ColumnPanel>(pyModule, "ColumnPanel", {
        base: ClassicContainer,

        properties: PyDefUtils.assembleGroupProperties<ColumnPanel>(
            ...
            ],
            {
                ...
                col_spacing: /*!componentProp(ColumnPanel)!1*/ {
                    ...
                    set(s, e, pyV) {
                        const v = pyV.toString();
                        const prefix = getCssPrefix();
                        for (const i of ["none", "tiny", "small", "medium", "large", "huge"]) {
                            e.classList.toggle(prefix + "col-padding-" + i, v === i);
                        }
        ...
        element({ col_spacing, ...props }) {
            ...
            return (
                <PyDefUtils.OuterElement className={`${prefix}column-panel anvil-container ${colSpacing}`} {...props} />
            );
        },
        ...
        locals($loc) {
            $loc["__new__"] = PyDefUtils.mkNew<ColumnPanel>(ClassicContainer, (self) => {
            ...
            $loc["clear"] = new Sk.builtin.func(function (self: ColumnPanel) {
                const superClear = ClassicContainer.tp$getattr<pyCallable>(new pyStr("clear"));
            ...
            */
            ...
            $loc["add_component"] = PyDefUtils.funcWithKwargs(function (
                ...
                return Sk.misceval.chain(component.anvil$hooks.setupDom(), (_rawComponentElement) => {
                    ...
                    const childIdx = index ?? self._anvil.components.length;
                    const gridPos = layoutProps.grid_position ? layoutProps.grid_position.toString() : "";

                    if (component._anvil) {
                        // this had better be for the designer's benefit only
                        component._anvil.layoutProps = layoutProps;
                    }

                    const _add = (component: Component, layoutProps: any, childIdx: number) => {
                        ...
                        const paddingClassName = `${prefix}col-padding belongs-to-${panelId} ${prefix}col-padding-${self._anvil.getPropJS(
                            "col_spacing"
                        )}`;
                        const [paddingElement] = (<div refName="padding" className={paddingClassName} />) as [
                            HTMLDivElement,
                            { padding: HTMLDivElement }
                        ];
                        if (currentColContainer === undefined) return;
                    ...
                    if (index == null) {
                        ...
                        withNewComponent.forEach(({ component, layoutProperties }, idx) =>
                            _add(component, layoutProperties, idx)
                        );
                    }

                    let prevEl: HTMLElement | null = null;

                    const cleanupClassData = () => {
                        if (!prevEl) return;
                        prevEl.classList.remove("belongs-to-" + panelId);
                        if (ANVIL_IN_DESIGNER) {
                            delete prevEl.dataset.anvilDesignerPanelChildIdx;
                            delete prevEl.dataset.anvilDesignerPanelGridPos;
                            delete prevEl.dataset.anvilDesignerColumnpanelComponent;
                        }
                    };
                    const applyClassData = () => {
                        const newEl = component.anvil$hooks.domElement!;
                        if (prevEl === newEl) {
                            return;
                        }
                        cleanupClassData();
                    ...
                    const beforeAdd = () => {
                        applyClassData();
                        return addEventHandler(component, s_x_anvil_dom_node_changed, applyClassData);
                    };
                    const afterRemoval = () => {
                        cleanupClassData();
                        return removeEventHandler(component, s_x_anvil_dom_node_changed, applyClassData);
                    };

                    return chainOrSuspend(beforeAdd(), () =>
                        pyModule["ClassicContainer"]._doAddComponent(self, component, kwargs, {
                            detachDom: () => {
                                const componentElement = component.anvil$hooks.domElement!;
    ...
    function setColumnWidths(self: ColumnPanel, row: HTMLElement) {
        // Set all the columns in this row to the right width.
        const clientFailMsg =
            "The col_widths property is created from the design layout and should not be called in code";
        let colWidths;
        try {
        ...
        const allCols = row.querySelectorAll<HTMLDivElement>(":scope >.anvil-panel-col");
    ...
    function updateSharedLayoutProps(self: ColumnPanel) {
        ...
        self._anvil.element.find(".anvil-component.belongs-to-" + self._anvil.panelId).each(function (_, el) {
            ...
            if (lps.full_width_row) {
                e.parents(".anvil-panel-section-container")
                    .first()
                    .addClass(prefix + "full-width-row");
            } else {
                e.parents(".anvil-panel-section-container")
                    .first()
                    .removeClass(prefix + "full-width-row");
            }
            let v = "transparent";
...
export default ColumnPanelFactory;
