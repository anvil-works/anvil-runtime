...
type LastPaginationEntry = [number, number, any, TemplateInstance];

interface PaginationArray extends Array<LastPaginationEntry> {
    pyIterator?: any;
    constructItemTemplate?: any;
}

interface RepeatingPanelAnvil {
    elements: { items: HTMLElement };
    itemsElement: JQuery<HTMLElement>;
    pyHiddenContainer: ClassicContainer<{}>;
    ...
    itemsCounter: number;
    lastPagination: PaginationArray;
    paginate: (child?: TemplateInstance) => Suspension | [number, any, Done];
...
const RepeatingPanelFactory = (pyModule: PyModMap, componentHelpers: ComponentHelpers) => {
    ...
    pyModule["RepeatingPanel"] = PyDefUtils.mkComponentCls<RepeatingPanel>(pyModule, "RepeatingPanel", {
        properties: PyDefUtils.assembleGroupProperties<RepeatingPanel>(
            /*!componentProps(RepeatingPanel)!2*/ ["appearance", "layout", "layout_margin", "tooltip", "user data"],
            {
                ...
                items: /*!componentProp(RepeatingPanel)!1*/ {
                    ...
                    set(s, e, v) {
                        return lockingCall(s, () => setItems(s, v));
                    },
                },
            }
        ),
        ...
        element: (props) => {
            ...
            return (
                <PyDefUtils.OuterElement
                    className={`anvil-designer-component-namespace ${prefix}repeating-panel`}
                    {...props}>
        ...
        locals($loc) {
            $loc["__new__"] = PyDefUtils.mkNew(ClassicComponent, (self: RepeatingPanel) => {
                ...
                self._anvil.pyHiddenContainer = pyCall<ClassicContainer<{}>>(ClassicContainer);
                ...
                self._anvil.lastPagination = [] as PaginationArray;
            ...
            $loc["get_components"] = new pyFunc(function get_components(self: RepeatingPanel) {
                return pyCallOrSuspend(self._anvil.pyHiddenContainer.tp$getattr<pyCallable>(s_get_components));
            });
            ...
            $loc["raise_event_on_children"] = funcFastCall(function raise_event_on_children(
                args: [RepeatingPanel, pyObject],
                kwargs: any
            ) {
                const [self, eventName] = args;
                return pyCallOrSuspend(
                    self._anvil.pyHiddenContainer.tp$getattr<pyCallable>(new pyStr("raise_event_on_children")),
                    [eventName],
                    kwargs
                );
            });
    ...
    const getItemAt = (self: RepeatingPanel, idx: number) => {
        if (!self._anvil.pyIterator) return undefined;

        self._anvil.itemCache ??= [];
        // If we already have this item in the cache, return it.

        if (idx < self._anvil.itemCache.length) return self._anvil.itemCache[idx];

        // We don't already have it in the cache, so walk along the iterator until we do, then return that.

        return chainOrSuspend(
            iterForOrSuspend(
                self._anvil.pyIterator,
                (pyItem, i) => {
                    self._anvil.itemCache.push(pyItem);

                    if (i >= idx) return new Break();
                    else return i + 1;
                },
    ...
    const removeAllTemplateInstancesAfter = (
        self: RepeatingPanel,
        firstTemplateInstanceToRemove: TemplateInstance | null
    ) => {
        if (checkNone(firstTemplateInstanceToRemove)) {
            return;
        }
        let removing = false;
        return iterArray(self._anvil.lastPagination, ([, , , templateInstance]) => {
            if (removing || templateInstance === firstTemplateInstanceToRemove) {
                removing = true;
                return pyCallOrSuspend(templateInstance.tp$getattr<pyCallable>(s_remove_from_parent));
            }
        });
    };

    const paginate = (self: RepeatingPanel, updatedChild: TemplateInstance | null = null) => {
        ...
        if (updatedChild?._anvil?.pagination) {
            const i = self._anvil.lastPagination.findIndex(
                ([, , , templateInstance]) => templateInstance == updatedChild
            );
            if (i >= 0) {
        ...
        const [stoppedIdx, itemsCounterWhenStopped, childStartAfter, childDone] = self._anvil.pagination.startAfter || [
            0,
        ...
        const addComponent = (pyC: TemplateInstance) => {
            // pyC is always a new component so we can skip validateChild

            return chainOrSuspend(
                pyC.anvil$hooks.setupDom(),
                (el) => {
                    self._anvil.elements.items.appendChild(el);
                    if (self._anvil.dataGrid && pyC._anvil) {
                        pyC._anvil.dataGrid = self._anvil.dataGrid;
                    }
                },
                () => adder(self._anvil.pyHiddenContainer, pyC, {}, { isMounted: false })
            );
        };
        ...
        const currentPagination: PaginationArray = [];
        ...
        return chainOrSuspend(
            ...
            () =>
                PyDefUtils.whileOrSuspend(
                    ...
                    () => {
                        ...
                        const lp = self._anvil.lastPagination[0] as LastPaginationEntry | undefined;
                        if (!lp) {
            ...
            () =>
                PyDefUtils.whileOrSuspend(
                    () => true,
                    () =>
                        chainOrSuspend(getItemAt(self, idx), (pyItem: pyObject | undefined) => {
                            ...
                            const [currentItemIdx, currentItemRowsDisplayed, currentItemStartAfter, templateInstance] =
                                self._anvil.lastPagination[idxOnPage] || [-1, 0, null, pyNone];
                            ...
                            if (currentItemIdx === idx) {
                                ...
                                if (templateInstance._anvil?.pagination) {
                                    if (
                                        ...
                                        (!startAfterThisComponent ||
                                            childStartAfter === templateInstance._anvil.pagination.startAfter)
                                    ) {
                                        ...
                                        chainFns.push((result: [number, any, any]) => {
                                            const [rows, stoppedAt, done] = result;
                                            ...
                                            if (done === "INVALID") {
                                                ...
                                                self._anvil.pagination.done =
                                                    self._anvil.pagination.done === true && done;
                                            }
                                ...
                                chainFns.push(
                                    () => {
                                        ...
                                        return chainOrSuspend(
                                            self._anvil.constructItemTemplate?.(pyItem),
                                    ...
                                    (pyComponent: TemplateInstance) => {
                                        return chainOrSuspend(addComponent(pyComponent), () => {
                                            if (pyComponent._anvil?.paginate) {
                                                ...
                                                return chainOrSuspend(pyComponent._anvil.paginate(), (r) => {
                                                    const [rowsDisplayed, stoppedAt, done] = r;
                            ...
                            return chainOrSuspend(undefined, ...chainFns);
                        })
                ),
            () => {
    ...
    const repaginateWithParent = (self: RepeatingPanel) => {
        const parent = self._anvil.parent?.pyObj;
    ...
    const lockingCall = (self: RepeatingPanel, fn: () => any) => {
        if (ANVIL_IN_DESIGNER) {
    ...
    const setItems = (s: RepeatingPanel, v: any) => {
        s._anvil.itemsCounter++;
        s._anvil.itemCache = [];
        if (checkNone(v)) {
            s._anvil.pyIterator = pyIter(new pyList([]));
        } else {
            s._anvil.pyIterator = pyIter(v);
        }
        return repaginateWithParent(s);
    };
};

export default RepeatingPanelFactory;
