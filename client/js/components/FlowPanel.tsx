...
import { chainOrSuspend, pyNone, pyStr, toJs, toPy } from "@Sk";
...
interface FlowPanelAnvil {
    elements: { gutter: HTMLDivElement };
}

interface FlowPanel extends ClassicContainer<FlowPanelAnvil> {}

const FlowPanelFactory = (pyModule: PyModMap) => {
    ...
    const setGap = (s: FlowPanel, e: HTMLElement, v: any) => {
        const prefix = getCssPrefix();
        GAP_VALUES.forEach((i) => {
            e.classList.toggle(prefix + "flow-spacing-" + i, v === i);
        });
    };

    pyModule["FlowPanel"] = PyDefUtils.mkComponentCls<FlowPanel>(pyModule, "FlowPanel", {
        base: ClassicContainer,

        properties: PyDefUtils.assembleGroupProperties<FlowPanel>(
            /*!componentProps(FlowPanel)!1*/ ["appearance", "user data", "layout", "layout_spacing", "tooltip"],
            {
                align: /*!componentProp(FlowPanel)!1*/ {
                    ...
                    set(s, e, pyV) {
                        ...
                        const justifyContent = j[v] || "flex-start";
                        s._anvil.elements.gutter.style.justifyContent = justifyContent;
                    },
                },
                vertical_align: /*!componentProp(FlowPanel)!1*/ {
                    ...
                    set(s, e, pyV) {
                        ...
                        ["top", "middle", "bottom", "full"].forEach((i) => {
                            e.classList.toggle(prefix + "vertical-align-" + i, v === i);
                        });
                    },
                },
                ...
                gap: /*!componentProp(FlowPanel)!1*/ {
                    ...
                    defaultValue: pyNone, // Means look at spacing prop, otherwise use "medium". pyNone breaks gendoc.
                    important: false,
                    priority: 0,
                    pyVal: true,
                    get(s, e) {
        ...
        element({ align, spacing, gap, vertical_align, ...props }) {
            ...
            align = "justify-content: " + (alignMap[align.toString()] || "flex-start") + ";";
            vertical_align = prefix + "vertical-align-" + (vertical_align.toString() || "top");

            return (
                <PyDefUtils.OuterElement
                    className={`${prefix}flow-panel anvil-container anvil-container-overflow ${gapClass} ${vertical_align}`}
                    {...props}>
        ...
        locals($loc) {
            const ContainerElement = ({
                ...
                const prefix = getCssPrefix();
                const visibleClass = !Sk.misceval.isTrue(visible) ? ` ${prefix}visible-false` : "";
                let style = "";
                if (width) {
                ...
                return (
                    <div
                        className={`${prefix}flow-panel-item anvil-always-inline-container` + visibleClass}
                        style={style}></div>
                );
            };
            ...
            $loc["add_component"] = PyDefUtils.funcWithKwargs(function (
                ...
                return chainOrSuspend(component.anvil$hooks.setupDom(), (domNode) => {
                    ...
                    if (
                        !containerElement ||
                        typeof containerElement !== "object" ||
                        !("appendChild" in containerElement)
                    ) {
                        throw new Error("ContainerElement did not return a valid DOM element");
                    }
                    const containerEl = containerElement as HTMLDivElement;
                    containerEl.appendChild(domNode);
                    if (typeof idx === "number") {
                        gutter.insertBefore(containerEl, gutter.children[idx]);
                        // fall through
                    } else {
                        gutter.appendChild(containerEl);
                    }
                    return ClassicContainer._doAddComponent(self, component, kwargs, {
                        ...
                        setVisibility(v: boolean) {
                            containerEl.classList.toggle(getCssPrefix() + "visible-false", !v);
                        },
                    });
                });
            });
...
export default FlowPanelFactory;
