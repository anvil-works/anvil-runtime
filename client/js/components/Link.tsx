...
import { chainOrSuspend, checkNone, checkString, isTrue, pyIsInstance, pyNone, pyStr, toJs } from "@Sk";
import PyDefUtils from "PyDefUtils";
import { ClassicComponentConstructor } from "./ClassicComponent";
import { ClassicContainer } from "./ClassicContainer";
import { ColumnPanelAnvil } from "./ColumnPanel";
import { isHandled, setHandled } from "./events";

interface LinkAnvil extends ColumnPanelAnvil {
    elements: { root: HTMLAnchorElement; holder: HTMLElement };
    domNode: HTMLAnchorElement;
    urlHandle?: { release(): void; getUrl(): string } | null;
    urlHandleName?: string;
    pageEvents: { add(): unknown; remove(): void };
}

interface Link extends ClassicContainer<LinkAnvil> {}
...
const LinkFactory = (pyModule: PyModMap) => {
    const ColumnPanel = pyModule["ColumnPanel"] as ClassicComponentConstructor;

    pyModule["Link"] = PyDefUtils.mkComponentCls<Link>(pyModule, "Link", {
        base: ColumnPanel,
        ...
        properties: PyDefUtils.assembleGroupProperties(
            /*!componentProps(Link)!1*/ ["text", "icon" /*"interaction",*/, "tooltip", "user data"],
            {
                text: {
                    ...
                    set(s: Link, e: HTMLElement, v: any) {
                        v = checkNone(v) ? "" : v.toString();
                        const prefix = getCssPrefix();
                        const { holder } = s._anvil.elements;
                ...
                url: /*!componentProp(Link)!1*/ {
                    ...
                    initialize: true,
                    set(self: Link, _e: HTMLElement, v: any) {
                        ...
                        } else if (isTrue(pyIsInstance(v, pyModule["Media"] as any))) {
                            return chainOrSuspend(PyDefUtils.getUrlForMedia(v), (h) => {
                ...
                text_padding: /*!componentProp(Link)!1*/ {
                    ...
                    defaultValue: pyNone,
                    priority: 0,
                    set(s: Link, _e: HTMLElement, v: any) {
                        setElementPadding(s._anvil.elements.holder, v);
                    },
                    getUnset(s: Link, _e: HTMLElement, v: any) {
                        return getUnsetPadding(s._anvil.elements.holder, toJs(v));
                    },
                },
            }
        ),
        ...
        element({ col_spacing, ...props }) {
            ...
            const initialText = (props.text = checkNone(props.text) ? "" : props.text.toString());
            ...
            return (
                <a
                    ...
                    <div
                        refName="holder"
                        className={`${prefix}link-text`}
                        style={
                            textPaddingStyle + `display: ${initialText ? "inline-block" : "none"}; ${underlineStyle}`
                        }>
        ...
        locals($loc) {
            $loc["__new__"] = PyDefUtils.mkNew(ColumnPanel, (self: Link) => {
                self._anvil.element.on("click", (evt) => {
                    const e = evt;
    ...
    function setUrl(self: Link, url: string, name: string | null = null) {
