...
CREATE INDEX scheduled_tasks_app_id_idx ON scheduled_tasks (app_id, job_id);

-- Background tasks

CREATE TYPE background_task_status AS ENUM ('completed', 'threw', 'killed', 'mia');

CREATE TABLE background_tasks (app_id text not null,
                          id text not null,
                          session bigint,
                          task_name text,
                          routing jsonb,
...
CREATE INDEX background_tasks_idx ON background_tasks (app_id,id);
